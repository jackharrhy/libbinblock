...
test('complete reference set maps source folders into composable semantic stages', async () => {
  const archive = {
    files: [
      ...
      { path: 'blue 64-8 24 bit/col_blue_hi.png' },
  ...
  assert.deepEqual(result.outputs.map((output) => output.path), [
    'flat-color/col_blue_hi.png',
    'downscaled/col_blue_hi.png',
    'ordered-results/ColBinSet_0000.png',
  ]);
  assert.deepEqual(result.outputs.map((output) => output.properties.reproduction), [
...
test('flat-color recipe expands arbitrary palette values into analytic artifacts', async () => {
  const result = await compileRecipe(createFlatColorRecipe([
    ['ink', '#123456'],
    ['paper', '#fedcba'],
  ]));
  assert.deepEqual(result.outputs.map((output) => output.path), ['flat-color/ink.png', 'flat-color/paper.png']);
...
test('ordered-result recipe records 960 pixel aliases and 12 raster exceptions', async () => {
  ...
  assert.equal(recipe.stages.find((stage) => stage.id === 'ordered-results').forEach.alias.values.length, 960);
  assert.equal(recipe.stages.find((stage) => stage.id === 'ordered-result-exceptions').forEach.file.values.length, 12);
});
...
test('custom permutation recipe emits flat, mask, variant, and alias stages', async () => {
  ...
  assert.deepEqual([...variants[0].image.pixels], [...renderBlock({ width: 64, height: 64, base: '#00ffff', overlay: '#000000', opacity: 1, preset: 'top-down', rotation: 0 })]);
