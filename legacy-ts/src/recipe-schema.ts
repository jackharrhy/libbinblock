// Historical behavioral oracle only. JSON recipes are not the production IR.
...
export type Expression =
  ...
  | ['coalesce' | 'concat' | 'min' | 'max', ...ExpressionValue[]]
  | ['==' | '!=' | '<' | '<=' | '>' | '>=' | '+' | '-' | '*' | '/', ExpressionValue, ExpressionValue]
  | ['clamp', ExpressionValue, ExpressionValue, ExpressionValue]
  | ['to-string', ExpressionValue]
  | ['pad', ExpressionValue, ExpressionValue, ExpressionValue?]
  | ['case' | 'match' | 'let', ...ExpressionValue[]];
export type ExpressionValue = JsonPrimitive | Expression;
export type NumberValue = number | Expression;
export type IntegerValue = number | Expression;
export type StringValue = string | Expression;
export type ColorValue = string | Expression;

export const LiteralExpressionSchema: z.ZodType<['literal', JsonValue]> = z.tuple([z.literal('literal'), JsonSchema]);

function variadicExpression(operator: 'coalesce' | 'concat' | 'min' | 'max', minimumArguments = 1): z.ZodType<Expression> {
  return z
    .tuple([z.literal(operator)], ExpressionValueSchema)
    .refine(
...
function binaryExpression(operator: '==' | '!=' | '<' | '<=' | '>' | '>=' | '+' | '-' | '*' | '/'): z.ZodType<Expression> {
...
export const ExpressionSchema: z.ZodType<Expression> = z.lazy(() =>
  z.union([
    ...
    z.union([z.tuple([z.literal('get'), z.string().min(1)]), z.tuple([z.literal('get'), z.string().min(1), ExpressionValueSchema])]),
    ...
    z.tuple([z.literal('let')], ExpressionValueSchema).superRefine((expression, context) => {
      ...
      for (let index = 1; index < expression.length - 1; index += 2) {
        if (typeof expression[index] !== 'string')
          context.addIssue({ code: 'custom', path: [index], message: 'let binding names must be strings.' });
      }
    }),
  ]),
);

export const ExpressionValueSchema: z.ZodType<ExpressionValue> = z.lazy(() => z.union([PrimitiveSchema, ExpressionSchema]));
export const NumberValueSchema: z.ZodType<NumberValue> = z.union([z.number().finite(), ExpressionSchema]);
export const IntegerValueSchema: z.ZodType<IntegerValue> = z.union([z.number().int(), ExpressionSchema]);
export const StringValueSchema: z.ZodType<StringValue> = z.union([z.string(), ExpressionSchema]);
...
export type ImageNode =
  | { op: 'input'; binding: string }
  | { op: 'use'; definition: string; with: Record<string, ExpressionValue> }
  | { op: 'fill'; width: IntegerValue; height: IntegerValue; color: ColorValue }
  | {
      op: 'gradient';
      shape: 'preset';
      width: IntegerValue;
  ...
  | { op: 'reference-set/alpha-map'; index: IntegerValue; rasterAsset?: StringValue }
  | {
      op: 'glyph';
      font: StringValue;
      text: StringValue;
  ...
  | {
      ...
      sourceBackground: ColorValue;
      foreground: ColorValue;
      background: ColorValue;
    }
  | { op: 'shift-rgb'; source: ImageNode; sourceBase: ColorValue; targetBase: ColorValue }
  | { op: 'apply-mask'; source: ImageNode; mask: ImageNode; mode: 'multiply' | 'replace' }
  | { op: 'rotate'; source: ImageNode; turns: IntegerValue }
  | { op: 'resize'; source: ImageNode; width: IntegerValue; height: IntegerValue; filter: 'lanczos3' }
  | { op: 'crop'; source: ImageNode; x: IntegerValue; y: IntegerValue; width: IntegerValue; height: IntegerValue };
...
export const ImageNodeSchema: z.ZodType<ImageNode> = z.lazy(() =>
  z.union([
    ...
    z.strictObject({
      ...
      with: z.record(PropertyNameSchema, ExpressionValueSchema).default({}),
    }),
...
export type ValuesAxis = z.infer<typeof ValuesAxisSchema>;
export type StageAxis = z.infer<typeof StageAxisSchema>;
...
export type AliasStage = z.infer<typeof AliasStageSchema>;
export type Stage = z.infer<typeof StageSchema>;
export type RecipeOutput = z.infer<typeof OutputSchema>;

function visitImage(node: ImageNode, visitor: (node: ImageNode) => void): void {
  visitor(node);
  ...
  if ('source' in node) visitImage(node.source, visitor);
  if (node.op === 'apply-mask') visitImage(node.mask, visitor);
}

type RecipeDocumentShape = {
...
function validateRecipeReferences(document: RecipeDocumentShape, context: z.RefinementCtx): void {
  const definitionDependencies = new Map<string, string[]>(Object.keys(document.definitions).map((id) => [id, []]));
  for (const [definitionId, definition] of Object.entries(document.definitions))
    visitImage(definition.image, (node) => {
      if (node.op === 'use') {
        definitionDependencies.get(definitionId)?.push(node.definition);
        if (!(node.definition in document.definitions))
          context.addIssue({
            code: 'custom',
            path: ['definitions', definitionId, 'image'],
            message: `Unknown definition: ${node.definition}`,
          });
      }
      if (node.op === 'raster' && typeof node.asset === 'string' && document.assets[node.asset]?.type !== 'raster')
        context.addIssue({ code: 'custom', path: ['definitions', definitionId, 'image'], message: `Unknown raster asset: ${node.asset}` });
      if (node.op === 'glyph' && typeof node.font === 'string' && document.assets[node.font]?.type !== 'font')
        context.addIssue({ code: 'custom', path: ['definitions', definitionId, 'image'], message: `Unknown font asset: ${node.font}` });
    });

  const stages = new Map<string, Stage>();
  document.stages.forEach((stage, index) => {
    if (stages.has(stage.id))
      context.addIssue({ code: 'custom', path: ['stages', index, 'id'], message: `Duplicate stage id: ${stage.id}` });
    stages.set(stage.id, stage);
  });

  const dependencies = new Map<string, string[]>(document.stages.map((stage) => [stage.id, []]));
  document.stages.forEach((stage, stageIndex) => {
    ...
    for (const [binding, axis] of Object.entries(stage.forEach)) {
      ...
      if (!stages.has(axis.stage))
        context.addIssue({
          code: 'custom',
          path: ['stages', stageIndex, 'forEach', binding, 'stage'],
          message: `Unknown stage: ${axis.stage}`,
        });
    }
    if (stage.type === 'alias' && !bindings.has(stage.target.binding)) {
      context.addIssue({
        code: 'custom',
        path: ['stages', stageIndex, 'target', 'binding'],
        message: `Unknown binding: ${stage.target.binding}`,
      });
    }
    if (stage.type === 'render')
      visitImage(stage.image, (node) => {
        if (node.op === 'input' && !bindings.has(node.binding))
          context.addIssue({ code: 'custom', path: ['stages', stageIndex, 'image'], message: `Unknown input binding: ${node.binding}` });
        if (node.op === 'use' && !(node.definition in document.definitions))
          context.addIssue({ code: 'custom', path: ['stages', stageIndex, 'image'], message: `Unknown definition: ${node.definition}` });
        if (node.op === 'raster' && typeof node.asset === 'string' && document.assets[node.asset]?.type !== 'raster')
          context.addIssue({ code: 'custom', path: ['stages', stageIndex, 'image'], message: `Unknown raster asset: ${node.asset}` });
        if (node.op === 'glyph' && typeof node.font === 'string' && document.assets[node.font]?.type !== 'font')
          context.addIssue({ code: 'custom', path: ['stages', stageIndex, 'image'], message: `Unknown font asset: ${node.font}` });
      });
  });

  for (const [outputIndex, output] of document.outputs.entries()) {
    if (!stages.has(output.stage))
      context.addIssue({ code: 'custom', path: ['outputs', outputIndex, 'stage'], message: `Unknown output stage: ${output.stage}` });
  }

  const visiting = new Set<string>();
  const visited = new Set<string>();
  function visitStage(stageId: string, path: string[] = []): void {
  ...
  function visitDefinition(definitionId: string, path: string[] = []): void {
...
export type RecipeValidationIssue = z.core.$ZodIssue;
export type RecipeValidationResult = z.ZodSafeParseResult<RecipeDocument>;
